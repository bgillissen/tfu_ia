...
param ["_aoCoord"];
private ["_side", "_faction", "_units", "_group", "_randomPos", "_veh", "_groupPath", "_infGroup", "_buildings"];
...
if ( PARAMS_AO_groupPatrol > 0 ) then {
	for "_x" from 1 to PARAMS_AO_groupPatrol do {
		...
		_group = [_randomPos, _faction, (_groupPath + _infGroup) call BIS_fnc_configPath] call BIS_fnc_spawnGroup;
...
if ( PARAMS_AO_sniperPatrol > 0 ) then {
	for "_x" from 1 to PARAMS_AO_sniperPatrol do {
		...
		_group = [_randomPos, _faction, [selectRandom IA_sniperGroups] call Bis_fnc_getCfgSubClasses] call BIS_fnc_spawnGroup;
...
if (PARAMS_AO_garrison > 0) then {
	...
	if ( (count _buildings) > 6 ) then {
		...
		for "_x" from 1 to PARAMS_AO_garrison) do {
			...
			[_unit, _building, true] call common_fnc_garrisonUnit;
			_units append _unit;
		};
