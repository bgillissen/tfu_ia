class common {
	tag = "common";
	class functions {
		class arsenalAuto { file="common\arsenalAuto.sqf"; };
		class deleteObjects { file="common\deleteObjects.sqf"; };
		class garrisonUnit { file="common\garrisonUnit.sqf"; };
		class globalHint { file="common\globalHint.sqf"; };
		class globalNotification { file="common\globalNotification.sqf"; };
		class globalSideChat { file="common\globalSideChat.sqf"; };
			class implentAllowedVehicle { file="common\implentAllowedVehicle.sqf"; };
		class implentArsenal { file="common\implentArsenal.sqf"; };
		class implentAssets { file="common\implentAssets.sqf"; };
			class implentBaseVehicle { file="common\implentBaseVehicle.sqf"; };
		class implentCargo { file="common\implentCargo.sqf"; };
		class implentCond { file="common\implentCond.sqf"; };
			class implentRestrictedGear { file="common\implentRestrictedGear.sqf"; };
			class implentSpawn { file="common\implentSpawn.sqf"; };
			class implentSupply { file="common\implentSupply.sqf"; };
		class setCargo { file="common\setCargo.sqf"; };
		class setSkill { file="common\setSkill.sqf"; };
	};
};

