...
"tcb_ais_in_agony" addPublicVariableEventHandler {
	...
	private _in_agony = (_this select 1) select 1;
	//private _side = _unit getVariable "tcb_ais_side";
	//if (playerSide == _side) then {
		if (_in_agony) then {
			diag_log "agonyPVEH: adding actions";
			//_unit playActionNow "agonyStart";
			[_unit, "agonyStart", 2] call common_fnc_doGesture;
			if ( _unit == player ) then {
				["HQ", format["%1 is down and needs help at %2!",name _unit, mapGridPosition _unit]] call common_fnc_globalSideChat; 
			};
			if (isNil {_unit getVariable ["fa_action", Nil]}) then {
				private _fa_action = _unit addAction [format["<t color='#F00000'>First Aid to %1</t>",name _unit], {_this spawn tcb_fnc_firstAid},_unit, 100, false, true, "",
			...
			if (isNil {_unit getVariable ["drag_action", Nil]}) then {
				private _drag_action = _unit addAction [format["<t color='#FC9512'>Drag %1</t>",name _unit], {_this spawn tcb_fnc_drag},_unit, 99, false, true, "",
			...
			if (isNil {_unit getVariable ["carry_action", Nil]}) then {
				private _carry_action = _unit addAction [format["<t color='#FC9512'>Carry %1</t>",name _unit], {_this spawn tcb_fnc_carry}, _unit, 98, false, true, "",
				    "{not isNull (_target getVariable _x)} count ['healer','dragger','helper','carrier'] == 0 && {alive _target} && {vehicle _target == _target}"                                   ];
			...
			if (tcb_ais_medical_education >= 1) then {
				if (isNil {_unit getVariable ["help_action", Nil]}) then {
					private _help_action = _unit addAction [format["<t color='#FC9512'>Press on the wound</t>",name _unit], {_this spawn tcb_fnc_help}, _unit, 97, false, true, "",
						"{not isNull (_target getVariable _x)} count ['healer','dragger','helper','carrier'] == 0 && {alive _target} && {vehicle _target == _target}"];
