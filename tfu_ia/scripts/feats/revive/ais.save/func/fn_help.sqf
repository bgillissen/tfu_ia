...
if (!isNull(_injuredperson getVariable "healer") || {!isNull(_injuredperson getVariable "dragger")}) exitWith {
	[format ["%1 is being attended to." call tcb_ais_font, name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText
};
if (_injuredperson distance _helper > 2.5) exitWith {
	[format ["%1 is too far away to press on the wound." call tcb_ais_font, name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText
};
...
[_helper, "AinvPknlMstpSlayWrflDnon_1"] call common_fnc_doAnim;
...
if (alive _helper && {!(_helper getVariable "tcb_ais_agony")}) then {
	...
	[_helper, "AinvPknlMstpSlayWrflDnon_AmovPknlMstpSrasWrflDnon"] call common_fnc_doAnim;
};
