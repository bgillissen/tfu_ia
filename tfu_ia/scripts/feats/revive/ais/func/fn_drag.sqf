...
diag_log format["ais_fnc_Drag: %1", _this];

if (!isNull(_injuredperson getVariable "healer") || {!isNull(_injuredperson getVariable "dragger")} || {!isNull(_injuredperson getVariable "helper")}) exitWith {
	[format ["%1 is already being attended to.", name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText
};
if (_injuredperson distance _dragger > 2.5) exitWith {
	[format ["%1 is out of range to be dragged." call tcb_ais_font, name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText
};
...
[_injuredperson, "AinjPpneMrunSnonWnonDb"] call common_fnc_switchMove;
