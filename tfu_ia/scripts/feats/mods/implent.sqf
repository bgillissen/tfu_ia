...
0 = {
	...
	if ( ["gear", _conf, _sides, _checkMods, false] call mods_fnc_condition ) then {
		//arsenal gear
		...
		if (!isNil "_aK" ) then {
			[(_src select _aK select A__BACKPACKS), 
			 ...
			 (_src select _aK select A__AMMO) ] call mods_fnc_implentArsenal;
		};
		//------------------------------------------------------------ vehicle cargo
		...
		if ( !isNil "_vcK" ) then {
			{
				[_forEachIndex, 
				 ...
				 (_src select _vcK select _forEachIndex select VC__AMMO) ] call mods_fnc_implentVehicleCargo;
	...
	if ( ["bv", _conf, _sides, _checkMods, false] call mods_fnc_condition ) then {
		//------------------------------------------------------------ base vehicle
		{
			private _bvK = missionNamespace getVariable format["BV_%1", toUpper(_k)];
			if ( !isNil "_bvK" ) then {
				[_forEachIndex, (_src select _bvK select _forEachIndex)] call mods_fnc_implentBaseVehicle;
			};
	...
	[_sdBackpacks, 
	 ...
	 _sdAmmo,
	 _sdCrates,
	 _sdVehicles ] call mods_fnc_implentSupplyDrop;
	//------------------------------------------------------------ rewards
	if ( ["reward", _conf, _sides, _checkMods, false] call mods_fnc_condition ) then {
		private _rK = missionNamespace getVariable format["R_%1", toUpper(_k)];
		if ( !isNil "_rK" ) then {
			[(_src select _rK)] call mods_fnc_implentReward;	
		};
	...
	if ( PLAYER_SIDE in _sides ) then {
		//------------------------------------------------------------ players
		{
			...
			if ( !isNil "_rlK" ) then {
				[_forEachIndex, (_src select _rlK select _forEachIndex)] call mods_fnc_implentRoleLoadout;
			};
		...
		//------------------------------------------------------------ base atmosphere  role loadout
		{
			...
			if ( !isNil "_baloK" ) then {
				[_forEachIndex, (_src select _baloK select _forEachIndex)] call mods_fnc_implentRoleLoadout;
			};
	...
	if ( !(_restrictEnemyGear && _isEnemy) ) then {
		...
		if ( !isNil "_rgK" ) then {
			[(_src select _rgK select RG__LAUNCHER), 
			 ...
			 (_src select _rgK select RG__BACKPACK)] call mods_fnc_implentRestrictedGear;
		};
	};
	//------------------------------------------------------------ allowed vehicle
	...
	if ( !isNil "_avK" ) then {
		[(_src select _avK select AV__HELI), 
		 ...
		 (_src select _avK select AV__TANK)] call mods_fnc_implentAllowedVehicle;
	};
	//------------------------------------------------------------ enemy spawn
	if ( ["spawn", _conf, _sides, _checkMods, true] call mods_fnc_condition ) then {
		{
			private _sK = missionNamespace getVariable format["S_%1", toUpper(_k)];
			if ( !isNil "_sK" ) then {
				[_forEachIndex, (_src select _sK select _forEachIndex), (_sides select 0)] call mods_fnc_implentSpawn;
			};
			true
