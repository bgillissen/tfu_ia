...
//spawn units
_groups = [_flatPos] call SIDE_fnc_placeEnemies;

//briefing
[_markerCoord, format["Side Mission: %1", SIDE_hqCoastTitle], SIDE_size] call SIDE_fnc_placeMarkers;
[format["%1%2", format[SIDE_briefing, SIDE_hqCoastTitle], SIDE_hqCoastBriefing]] remoteExec ["common_fnc_globalHint", 0, false];
["NewSideMission", SIDE_hqCoastTitle] remoteExec ["common_fnc_globalNotification" ,0 , false];

while ( true ){
	if (!alive _hq) exitWith {
		[SIDE_hqCoastFailHint] remoteExec ["common_fnc_globalHint", 0, false];
		[false, _flatPos, _groups, [_boat, _trawler, _crate, _hq]] spawn SIDE_fnc_cleanup;
	};
	if ( SIDE_success ) exitWith {
		[SIDE_hqCoastPlanted] remoteExec ["common_fnc_globalSideChat", 0, false];
		sleep 30;
